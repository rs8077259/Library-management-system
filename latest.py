...
def chart():
    ...
    charts.minsize(1300,800)
    charts.maxsize(1300,800)
    title=tk.Label(charts,text='CHARTS',font=('arial black',35),fg='red',bg='white')
...
def b_serch(l):
    ...
    listbo2.place(x=480,y=520,width=303)
    listbo2.yview()
...
def book_window():
    ...
    book_table.column('Number_Of_Pages',anchor='center',width=200)
    book_table.column('Book no',anchor='center',)
    book_table.column('Book_title',anchor='center',width=570)
    book_table.column('Type',anchor='center',)
    book_table.column('Price',anchor='center',width=90)
    book_table.column('Book Detail',anchor='center',width=190)
    book_table.column('Status',anchor='center',width=180)
    for item_ in head:
    ...
    book_no=tk.Label(add_b,text='Book No',).place(x=100,y=90)
    book_no=tk.Entry(add_b,)
    book_no.place(x=170,y=90)
    book_no.bind('<KeyRelease>',identyfy)

    # book name _____________________________________________________________________________
    book_name=tk.Label(add_b,text='Book Name',).place(x=370,y=90)
    book_name=tk.Entry(add_b,width=67)
    book_name.place(x=450,y=90)

    #page____________________________________________________________________________________
    pages=tk.Label(add_b,text='Pages',).place(x=910,y=90)
    pages=tk.Entry(add_b,width=30)
    pages.place(x=960,y=90)

    #type __________________________________________________________________________________
    type_=tk.Label(add_b,text='Cover_Type',).place(x=1200,y=90)
    type_=tk.Entry(add_b,width=40)
    type_.place(x=1290,y=90)
    type_.insert(0,'Paperback')

    #price___________________________________________________________________________________
    price=tk.Label(add_b,text='Price',).place(x=420,y=170)
    price=tk.Entry(add_b,width=40)
    price.place(x=460,y=170)

    # Book Detailtable_______________________________________________________________________
    book_d=tk.Label(add_b,text='Book Type',).place(x=760,y=170)
    book_d=tk.Entry(add_b,width=45)
    book_d.place(x=840,y=170)

    # button of book table___________________________________________________________________
    button=tk.Button(add_b,text='Add',command=add_book,borderwidth=2,font=('arial black',10),activeforeground='white',activebackground='green').place(x=1500,y=240)
    button7=tk.Button(add_b,text='Upgrade',command=upgrade_book,borderwidth=2,font=('arial black',10),activeforeground='white',activebackground='orange').place(x=1400,y=240)

    button2=tk.Button(add_b,text='Delete',command=delete_book,borderwidth=2,font=('arial black',10),activeforeground='white',activebackground='red').place(x=1300,y=240)
    add_b.place(x=0,y=519)
    button6=tk.Button(add_b,text='<-- Back ',borderwidth=2,font=('arial black',10),activeforeground='white',activebackground='red',command=lambda : main_window(root.destroy())).place(x=20,y=240)
